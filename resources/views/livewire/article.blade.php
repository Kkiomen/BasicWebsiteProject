<div>
    <section class="container-sm mx-auto px-4 mt-5">
        <article>
            <div class="mb-6 text-center">
                <p class="category-article-title"><a
                        href="{{ route('category', ['category_translate' => \Illuminate\Support\Facades\Lang::get('basic.category'), 'category_slug' => $category_slug]) }}">{{ $category_name }}</a>
                </p>
    ...
    <section class="bg-[#0466c8] text-white">
        <div class="container-sm mx-auto px-4 py-12 mt-5">
            <div class="text-center">
                <h2 class="mb-2 text-3xl lg:text-3xl tracking-tight font-semibold text-white dark:text-white">Zadania</h2>
                <p class="font-light text-white sm:text-sm dark:text-gray-400">Przećwicz zdobytą wiedzę</p>
                <hr class="mt-4 mb-6"/>
            </div>
            <div>
                ...
                @foreach($tasks as $task)
                    @if($loop->index == 0)
                        <div class="" x-data="{ open{{$loop->index}}: false }">
                            <div class="px-4 text-lg select-none cursor-pointer" @click="open{{$loop->index}} = ! open{{$loop->index}}">
                                {{ $task->name }}
                            </div>
                    @else
                        <div class="mt-6" x-data="{ open{{$loop->index}}: false }">
                            <hr/>
                            <div class="px-4 mt-4 text-lg select-none cursor-pointer" @click="open{{$loop->index}} = ! open{{$loop->index}}">
                                {{ $task->name }}
                            </div>
                    @endif
                            ...
                            <div class="mt-6 border-t border-gray-100">
                                <dl class="divide-y divide-gray-100">
                                    <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                                        <dt class="text-sm font-medium leading-6 text-white">Opis zadania</dt>
                                    ...
                                    <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0" x-data="{ hint{{$loop->index}}: false }">
                                        <dt class="text-sm font-medium leading-6 text-white">Podpowiedź</dt>
                                        <dd class="mt-1 text-sm leading-6 text-white sm:col-span-2 sm:mt-0">
                                            <div x-show="!hint{{$loop->index}}" @click="hint{{$loop->index}} = true" @click.outside="hint{{$loop->index}} = false">
                                                <i class="fa-solid fa-eye-slash me-3" ></i> Kliknij aby zobaczyć
                                            </div>
                                    ...
                                    <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0" x-data="{ solution{{$loop->index}}: false }">
                                        <dt class="text-sm font-medium leading-6 text-white">Rozwiązanie</dt>
                                        <dd class="mt-1 text-sm leading-6 text-white sm:col-span-2 sm:mt-0">
                                    ...
                                    <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0" x-data="{ explained{{$loop->index}}: false }">
                                        <dt class="text-sm font-medium leading-6 text-white">Wytłumaczenie</dt>
                                        <dd class="mt-1 text-sm leading-6 text-white sm:col-span-2 sm:mt-0">
        ...
        <div class="py-8 px-4 mx-auto max-w-screen-xl lg:py-16 lg:px-6">
            <div class="mx-auto max-w-screen-sm text-center lg:mb-16 mb-8">
                <h2 class="mb-4 text-3xl lg:text-4xl tracking-tight font-extrabold text-gray-900 dark:text-white">
                    Pozostałe artykuły</h2>
                <p class="font-light text-gray-500 sm:text-xl dark:text-gray-400">Sprawdź moje inne artykuły. Napewno
